...
**Điều hướng nhanh:** tab **Quầy / Quản lý** (6.2a); mô tả **giao diện sau khi làm xong** và **Wizard** ở **mục 11–12** (cuối tài liệu).
...
| **Menu**                   | **Tệp** → Cài đặt, Mở thư mục nhật ký phiên. **Mục tiêu sau refactor:** «Tìm kiếm / quản lý video» chuyển sang **tab** (mục **6.2a**), không giữ là mục menu chính.                                                                                                           |
...
- **Chế độ Quầy (Kiosk / Daily):** chỉ preview lớn, chip trạng thái («Chờ quét» / «Đang ghi: …»), tên quầy nhỏ, **một** lối rõ ràng (nút / menu) để vào lại Thiết lập (Wizard hoặc Cài đặt) — **không** dùng PIN hay phím tắt riêng để «mở khóa» Setup.
...
### 6.2a. Tab «Quầy» và tab «Quản lý / tìm kiếm video»

**Vấn đề:** đưa «tìm kiếm video» lên **thanh công cụ header** (cạnh Ghim, đường dẫn lưu, độ phân giải) dễ **lẫn** với thao tác vận hành quầy và tăng nhiễu khi quét liên tục.

**Định hướng đã chốt:**

- Dùng **thanh tab** (ví dụ `QTabWidget` hoặc dải nút tab ngay **dưới** menu bar / phía **trên** vùng nội dung), **không** nằm trên **header toolbar** của Quầy.
- **Hai tab tối thiểu:** **«Quầy»** (mặc định — preview, trạng thái, quét → quay) và **«Quản lý»** hoặc **«Tìm kiếm video»** (nội dung tương đương `RecordingSearchDialog` / lịch sử, truy vấn SQLite).
- **Một cú click** chuyển tab — không cần mở menu **Tệp**.
- **Menu Tệp:** có thể **bỏ** mục «Tìm kiếm video đã ghi» hoặc giữ như **lối phụ** (mở cùng nội dung tab Quản lý) — ưu tiên UX qua tab.

**Luồng quét → quay (không bị ảnh hưởng):**
...
- Hiển thị tối đa:
  ...
  - Tên máy / quầy.
  - **Không** hiện combo COM/RTSP trên mặt Quầy trừ khi vào **Thiết lập** (Wizard / Cài đặt đầy đủ).
- Vào lại Thiết lập bằng nút **«Thiết lập máy & quầy»** và/hoặc **Tệp** → Wizard / Cài đặt (**mục 12**) — **không** yêu cầu PIN hay phím tắt.
...
- Chuyển sang tab **Quản lý / tìm kiếm**: quét → quay **vẫn hoạt động** (worker không dừng theo tab — **6.2a**).
...
### 11.2. Tab Quầy / Quản lý, menu và header (mục tiêu)

- **Tab (6.2a):** **«Quầy»** | **«Quản lý / tìm kiếm video»** — một click chuyển; **không** đặt tìm kiếm hay quản lý lên **thanh header** (toolbar) để không chen với Ghim / đường dẫn / độ phân giải.
- **Menu Tệp:** giữ **Cài đặt** (hộp thoại đầy đủ cho admin); thêm **«Trình hướng dẫn thiết lập quầy…»** (mở Wizard) — chi tiết **mục 12.1**; **không** coi «Tìm kiếm video» là mục chính (đã có tab).
- **Thanh header Quầy:** **Ghim** (tuỳ chọn), **Đường dẫn lưu**, **Độ phân giải**, nút **«Thiết lập máy & quầy»** (Wizard) — **không** thêm nút tìm kiếm ở đây. **Làm mới thiết bị** trong Cài đặt hoặc Wizard.
...
- **Done:** tab Quầy/Quản lý, kiosk defaults, setup wizard (USB/RTSP + scanner flows), mini-overlay, startup shortcut, Winson QR/Code128, camera error dialog retry/setup.
- **Partially done:** wizard camera probing currently supports refresh + RTSP test-connect lightweight checks; no full live preview panel embedded in wizard step.
- **Not in scope:** pipeline architecture rewrite, full i18n expansion, cloud user/account management.
